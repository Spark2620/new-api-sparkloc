...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if tokenGroup != "" {
			if channelId, ok := model.ParseChannelGroupName(tokenGroup); ok {
				channel, err := model.GetChannelById(channelId, false)
				if err != nil || channel == nil || channel.Status != common.ChannelStatusEnabled || !constant.IsSupportedChannelType(channel.Type) {
					abortWithOpenAiMessage(c, http.StatusForbidden, fmt.Sprintf("channel %d has been disabled", channelId))
					return
				}
			} else {
				...
				if !ratio_setting.ContainsGroupRatio(tokenGroup) {
					if tokenGroup != "auto" {
						abortWithOpenAiMessage(c, http.StatusForbidden, fmt.Sprintf("group %s has been disabled", tokenGroup))
						return
					}
				}
			}
